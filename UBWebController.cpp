...
void UBWebController::webBrowserInstance()
{
	...
	else
	{
		...
		{
			mCurrentWebBrowser->loadUrlInNewTab(currentUrl);
...
void UBWebController::activePageChanged()
{
	if (mCurrentWebBrowser && mCurrentWebBrowser->currentTabWebView())
	{
		...
		{
			mTrapFlashController->updateTrapFlashFromPage(mCurrentWebBrowser->currentTabWebView()->page());
		}
		//                #endif
...
//void UBWebController::hasEmbeddedContent()
//{
////    bool bHasContent = false;
//    if(mCurrentWebBrowser){


//         mCurrentWebBrowser->currentTabWebView()->webPage()->toHtml([this](const QString &htmlpara)
//        {
//             QString html=htmlpara;
...
//}
// #if defined(QWEBENGINEPAGE_CAPTURECURRENTPAGE)
void UBWebController::captureCurrentPage()
{
		...
		&& mCurrentWebBrowser->currentTabWebView()->page())
	{
		...
		QWebEnginePage* frame = mCurrentWebBrowser->currentTabWebView()->page();
		QSize size = frame->contentsSize().toSize();
		qDebug() << size;

		frame->runJavaScript("document.getElementsByTagName('body')[0].clientTop", [this, frame](const QVariant &top) {
			frame->runJavaScript("document.getElementsByTagName('body')[0].clientLeft", [this, top, frame](const QVariant &left) {
				frame->runJavaScript("document.getElementsByTagName('body')[0].clientWidth", [this, top, left, frame](const QVariant &width) {
					frame->runJavaScript("document.getElementsByTagName('body')[0].clientHeight", [this, top, left, width, frame](const QVariant &height) {
						//                        QSize vieportSize = mCurrentWebBrowser->currentTabWebView()->page()->view()->size();
						//                        mCurrentWebBrowser->currentTabWebView()->page()-frame->contentsSize());
						QPixmap pix = QPixmap(frame->view()->geometry().width(), frame->view()->geometry().height());
						...
						//                        mCurrentWebBrowser->currentTabWebView()->page()->view()-->setSize(vieportSize);
						if (!pix.isNull())
							emit imageCaptured(pix, true, mCurrentWebBrowser->currentTabWebView()->url());
...
void UBWebController::setupPalettes()
{
	if (!mToolsCurrentPalette)
	{
		...
		// add for teacher by nick wu
		connect(mMainWindow->actionWebInteractiveCapture, SIGNAL(triggered()), this, SLOT(teacherCapture()));
...
// add for teacher by nick wu
void UBWebController::teacherCapture()
{
	mToolsCurrentPalette->setVisible(false);
	qApp->processEvents();
	...
	{
		QPixmap selectedPixmap = customCaptureWindow.getSelectedPixmap();
		emit imageTeacherCaptured(selectedPixmap, false, mCurrentWebBrowser->currentTabWebView()->url());
	}
...
void UBWebController::getEmbeddableContent()
{
	...
	if (mCurrentWebBrowser) {
		//        QNetworkAccessManager* pNam = mCurrentWebBrowser->currentTabWebView()->webPage()
		//        if(NULL != pNam){
		//            mCurrentWebBrowser->currentTabWebView()->webPage()->toHtml([this](const QString &htmlpara )
		//            {
...
void UBWebController::captureEduMedia()
{
	if (mCurrentWebBrowser && mCurrentWebBrowser->currentTabWebView())
	{
		...
		{
			//            QWebEngineScriptCollection objects = webView->page()->->runJavaScript();->findAllElements("object");
			webView->page()->runJavaScript("function myFunction() {var objects=document.body.querySelectorAll('object>param[name=flashvars]');"
				"var values=new Array();"
				"Array.prototype.slice.call(objects).forEach(function(element,index)"
				"{values.push(element.getAttribute('value'));});return values;}", [](const QVariant &values) {

				QStringList Values = values.toStringList();
